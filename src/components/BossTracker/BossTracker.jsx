...
import { colors } from "../utility/BossMappings"
...
function CreateDiff({ character, boss, setCharacters }) {
    return (
        Object.keys(character.bossSize[boss]).map((diff, index) => {
            return (<DiffBtns key={index} character={character} boss={boss} setCharacters={setCharacters} diff={diff} index={index} />)
        })
    )
}


function DiffBtns({ character, setCharacters, boss, diff, index }) {
